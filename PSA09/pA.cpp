...
bool HugeInteger::isEqualTo(HugeInteger& n){
    ...
    // TODO 5
    if (_sign != n._sign) return 0;
    for (int i = 0;i < _length; i++) if (data[i] != n.data[i]) return 0;
    return 1;
}
...
bool HugeInteger::isGreaterThan(HugeInteger& n){
    this->resize(); n.resize();
    // TODO 7
    if (this->isEqualTo(n)) return 0;
    if (_sign != n._sign) return _sign == 1 ? 0 : 1;
    bool check = -1;
    for (int i = 0;i < _length; i++) {
        if (data[i] > n.data[i] && check == -1) {
            check = 1;
            break;
        }
        else if (data[i] < n.data[i]) check = 0;
    }
    if (_sign == 1) return !check;
    else return check;
}
bool HugeInteger::isLessThan(HugeInteger& n){
    this->resize(); n.resize();
    // TODO 8
    return !(this->isGreaterThan(n) || this->isEqualTo(n));
}
