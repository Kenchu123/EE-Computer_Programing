...
class Player{
    public:
        struct hand_type{
            enum type{full_house=6,flush=5,straight=4,two_pair=3,one_pair=2,high_card=1}Type;
            card dominate_card;
        };
...
void Player::judge_hand(){
    ...
    /*two_pair*/
    for(int n1;n1<5;n1++)for(int n2;n2<5;n2++)for(int n3;n3<5;n3++)for(int n4;n4<5;n4++){
...
class RatedPlayer : public Player {
    ...
    public:
        RatedPlayer() {}
        RatedPlayer(const string n, const unsigned r = 1) {
            _name = n;
            _rate = r;
        }
        void show_Name_Rate() {
            // "Name:" + 名字 + " Rate:" + 排名 + 換行兩次
            cout << "Name:" << _name << " Rate:" << _rate << "\n" << endl;
        }
        void setRate(const unsigned r) {
            _rate = r;
        }
};
